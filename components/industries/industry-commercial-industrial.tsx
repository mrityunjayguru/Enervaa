...
import {
  commercialIndustrialDetailData,
  IndustryDetailSectionData,
} from "@/lib/site-data";
import { ArrowRight } from "lucide-react";

interface IndustryCommercialIndustrialSectionProps {
  data?: IndustryDetailSectionData;
  className?: string;
}

export default function IndustryCommercialIndustrialSection({
  data = commercialIndustrialDetailData,
  className = "",
}: IndustryCommercialIndustrialSectionProps) {
  return (
    <section
    ...
    >
      <div className="px-4 sm:px-6 lg:px-8 xl:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-5 lg:gap-6 mb-12 sm:mb-16">
          ...
          {data.caseStudies.map((item, idx) => (
            <AnimateIn key={idx} variant="fade-up" delay={150 + idx * 100}>
              ...
              >
                ...
                {/* Below-Box Caption */}
                <div className="pt-4 flex flex-col items-start flex-1 justify-between">
                  <div>
                    <p className="text-xs sm:text-[13px] font-semibold tracking-wider text-[#4A4D3A] uppercase mb-1">
                      {item.categoryTag}
                    </p>
                    <h3 className="text-2xl sm:text-3xl font-normal text-[#78BA43] transition-colors group-hover:text-[#67A337] mb-2">
                      {item.title}
                    </h3>
                    {item.description && (
                  ...
                  <div className="inline-flex items-center gap-1.5 text-sm sm:text-base font-medium text-[#78BA43] group-hover:text-[#68A535] transition-colors mt-2">
                    <span>{item.contextLinkText || "More Details"}</span>
