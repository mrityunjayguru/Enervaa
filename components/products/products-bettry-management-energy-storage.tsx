...
import { energyStorageManagementSystemsData, energyStorageSystemsData, highVoltageManagementData } from "@/lib/site-data";

export default function ProductsBettryManagementEnergyStorage() {
  return (
    <section className="w-full bg-[#F5F7EE] py-12 sm:py-16 lg:py-20 relative">
      <div className="px-4 sm:px-6 lg:px-8 xl:px-12">
          ...
          <div className="max-w-xl lg:pt-6">
            <AnimateIn variant="fade-up" delay={150}>
              <p className="text-[15px] sm:text-lg text-black mb-8 max-w-4xl">
                The three-tier ESS architecture: module slave, rack master, multi-string controller.
              </p>
            </AnimateIn>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10 mt-12 sm:mt-16">
          {energyStorageManagementSystemsData.map((product, idx) => (
            <AnimateIn
              key={product.id}
              variant="fade-up"
              delay={150 + idx * 80}
